...
interface EditTaskDialogProps {
  ...
  task: Task | null;
  onClose: () => void;
  onSubmit: (id: string, payload: { title: string; body: string }) => Promise<void>;
}
...
export const EditTaskDialog: React.FC<EditTaskDialogProps> = ({
  ...
  const handleFormSubmit = async (data: EditTaskFormValues) => {
    if (task) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit task" className="max-w-lg">
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label htmlFor="edit-body" className="text-sm font-semibold text-gray-700">
